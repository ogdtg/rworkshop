...
Mit `across()` (seit dplyr 1.0.0) können die gleiche Operation auf mehrere Spalten gleichzeitig angewendet werden.

> **Hinweis:** Die älteren Funktionen `mutate_all()` und `mutate_at()` sind veraltet (*deprecated*) und sollten durch `across()` ersetzt werden.

.panelset[
.panel[.panel-name[across – alle Spalten]
```{r, echo=TRUE}
df_tidy %>%
...
.panel[.panel-name[across – bestimmte Spalten]
```{r, echo=TRUE}
df_tidy %>%
  mutate(across(c(cases, deaths), ~ .x / 1000)) %>%
  head(3)
```
]
.panel[.panel-name[across – nach Typ]
```{r, echo=TRUE}
df_tidy %>%
...
Mit `slice_max()` / `slice_min()` können die Zeilen mit den höchsten (oder niedrigsten) Werten gefiltert werden.

> **Hinweis:** `top_n()` ist veraltet (*deprecated*) – bitte `slice_max()` / `slice_min()` verwenden.
