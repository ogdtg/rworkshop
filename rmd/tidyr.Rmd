...
> **Hinweis (tidyr ≥ 1.3.0):** `separate()` ist veraltet (*deprecated*). Die modernen Nachfolger sind `separate_wider_delim()` (für Trennzeichen) und `separate_wider_position()` (für feste Breiten).
...
Spalten können mit `separate_wider_delim()` in mehrere Spalten aufgeteilt werden.

> `separate()` ist veraltet – bitte `separate_wider_delim()` verwenden.

```{r, echo=TRUE}
df_tidy_m %>%
  tidyr::separate_wider_delim(col = date, delim = "-",
